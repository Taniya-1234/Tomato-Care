...
const ImageUpload = () => {
  ...
  // Save document and capture reference so we can update with analysis results
  const docRef = await addDoc(collection(db, "scans"), scanData);

  // Try to run analysis immediately and update the scan document with results
  try {
        // Backend expects base64 without data URL header OR accepts data URL; our API handles both.
        const apiResult = await TomatoDiseaseAPI.predictFromBase64(imageBase64);

        // Transform result to fields matching Firestore schema
        const updateFields = {
          status: apiResult.success ? "completed" : "failed",
          diseaseName: apiResult.disease || (apiResult.prediction && apiResult.prediction.disease) || null,
          confidence: apiResult.disease_confidence || apiResult.confidence || (apiResult.prediction && apiResult.prediction.confidence) || null,
          treatment: apiResult.recommendations || apiResult.treatment || null,
          analysisDate: serverTimestamp(),
        };
        // Update the saved document directly using the returned DocumentReference
        await updateDoc(docRef, updateFields);
      } catch (apiErr) {
        console.error("Analysis API error:", apiErr);
      }
